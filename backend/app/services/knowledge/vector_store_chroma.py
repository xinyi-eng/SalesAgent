    ...
    def search(self, query: str, top_k: int = 5, category: Optional[str] = None) -> List[SearchResult]:
        ...
        try:
            ...
            for i, doc_id in enumerate(results["ids"][0]):
                # 找到对应的chunk
                chunk = next((c for c in self._chunks if c.id == doc_id), None)
                if chunk:
...
def get_vector_store() -> VectorStore:
    ...
    if _vector_store is None:
        ...
        if use_milvus:
            try:
                from app.services.knowledge.vector_store_milvus import MilvusVectorStore
